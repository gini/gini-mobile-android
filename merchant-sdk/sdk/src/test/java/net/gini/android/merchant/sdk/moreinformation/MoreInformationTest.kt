...
class MoreInformationTest {
    ...
    fun `loads payment provider icons when returned from view model`() {
        ...
        // Then
        onView(withId(R.id.gms_payment_providers_icons_list)).check { view, _ -> assertThat ((view as RecyclerView).adapter!!.itemCount).isEqualTo(3) }
    }
    ...
    fun `displays all FAQ questions`() {
        ...
        // Then
        onView(withId(R.id.gms_faq_list)).check { view, _ -> assertThat ((view as ExpandableListView).adapter!!.count).isEqualTo(fragment.faqList.size) }
    }
}