...
class NoResultScreen {
    ...
    fun checkNoResultHeaderForQRCodeIsDisplay(){
        onView(withText(net.gini.android.capture.R.string.gc_noresults_header_qr)).check(matches(
            isDisplayed()
        ))
    ...
    fun clickRetakeImagesButton() {
        onView(withText(net.gini.android.capture.R.string.gc_noresults_retake_images)).perform(
            click())
    }
}