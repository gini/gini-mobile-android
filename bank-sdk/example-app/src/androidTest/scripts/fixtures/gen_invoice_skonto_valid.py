#!/usr/bin/env python3
"""Render a synthetic German invoice carrying a still-claimable skonto discount.

Fixture for the standalone Skonto screen tests (SkontoScreenTests).

Why the invoice date is in the future: a skonto window is relative to the invoice
date and only 7-14 days wide, so a real invoice's discount stops being claimable
about two weeks after it was issued. `GetSkontoEdgeCaseUseCase` treats any
`skontoDueDate` before today as `SkontoEdgeCase.SkontoExpired`, and
`GetSkontoDefaultSelectionStateUseCase` then switches the discount off. A 2028
invoice date is the only way a committed fixture keeps the discount on -- the same
trick `gen_invoice_future_due.py` uses for `paymentDueDate`.

Rechnungsdatum: 05.08.2028, 14-day window -> skonto deadline 19.08.2028.
Both dates must match SkontoFixtures in the tests.

Deliberately carries NO position table: with line items the Return Assistant claims
the flow and the standalone Skonto screen is never reached. The RA + skonto case has
its own fixture, `gen_invoice_skonto_ra_valid.py`.
"""
import os
...
y += 100
d.text((x, y), "Rechnung Nr. 2028-0431", font=f(40, True), fill="black")
y += 66
d.text((x, y), "Rechnungsdatum: 05.08.2028", font=f(26), fill="black")
y += 38
d.text((x, y), "Kundennummer: 44821", font=f(26), fill="black")

# One aggregated service charge, no numbered position table: a position table is what
# the backend turns into `lineItems`, and those would hand the flow to the Return
# Assistant before the Skonto screen appears.
y += 90
d.text((x, y), "Leistung", font=f(26, True), fill="black")
y += 44
d.line((x, y, x + 1000, y), fill="black", width=2)
y += 26
d.text((x, y), "Malerarbeiten Wohnzimmer, pauschal", font=f(26), fill="black")
y += 54
d.line((x, y, x + 1000, y), fill="black", width=2)
y += 26
...
y += 38
d.text((x, y), "Verwendungszweck: RE 2028-0431", font=f(26), fill="black")
...
out = os.path.join(os.path.dirname(__file__), "..", "..", "assets", "skonto_valid.jpeg")
