...
The Gini Bank API Library makes heavy use of the concept of tasks. Tasks are convenient when you want to execute work
in succession, each one waiting for the previous to finish (comparable to Promises in JavaScript). This is a common
pattern when working with Gini's remote APIs. The Gini Bank API Library uses `facebook's task implementation, which is
...
.. code-block:: java
    ...
    DocumentTaskManager documentTaskManager = giniBankApi.getDocumentTaskManager();
...
.. code-block:: java
        ...
        final DocumentTaskManager documentTaskManager = giniBankApi.getDocumentTaskManager();
...
Currently, the Gini Bank API Library doesn't have intelligent error-handling mechanisms. All errors that
