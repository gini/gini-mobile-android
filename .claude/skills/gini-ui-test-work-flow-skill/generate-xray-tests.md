...
> Never read `.kt` or `.java` source files or any other implementation file to infer behaviour.
...
- Never read `.kt` or `.java` source files, class names, data classes, or any implementation detail to write steps. If it is not visible on screen, it does not belong in a step.
...
|---|---|
| `bank-sdk` | `bank-sdk/example-app` ("Gini Bank") |
| `health-sdk` | `health-sdk/example-app` ("Gini Health") |
...
When the AC contains accessibility criteria (TalkBack, font scaling, contrast, landscape, external keyboard, …), decide their shape by the size of the UI under test:
- **Single small screen or component** (one screen, bottom sheet, dialog, banner): generate **one** consolidated accessibility test case. It navigates to the screen once, then verifies every accessibility criterion as sequenced observation steps. For criteria requiring device setup (TalkBack, largest font scale, external keyboard), order the steps as: apply the setting → observe → revert the setting before the next criterion. Put the setting in the `Data` column of each step.
...
- ✅ One `... - Accessibility - <screen> meets accessibility requirements` test case: reach the sheet once, then sequenced steps for font scaling, TalkBack, landscape, contrast, and keyboard, each with its own observation.
