...
| Value | SDK | Demo app used for test steps | Module path |
|---|---|---|---|
| `GiniBankSDKExample` | GiniBankSDK | Gini Bank | `bank-sdk/example-app` |
| `GiniHealthSDKExample` | GiniHealthSDK | Gini Health | `health-sdk/example-app` |
...
This skill is running in `gini-mobile-android` → platform is **android**.
Steps must use Android gestures (tap, swipe, back button), Android UI patterns, and Android screen navigation. Do not ask the user for this value.
...
> Never read `.kt`, `.java`, or any other implementation file to infer behaviour.
...
- **Platform-scoped.** This is an Android repository — read `res/values/strings.xml` inside the relevant module (e.g. `bank-sdk/example-app/src/main/res/values/strings.xml`). Skip if strings are unavailable or would make the test platform-specific.
...
- All steps are written from the perspective of a tester interacting with the product's demo app on Android.
- Never read `.kt`, `.java`, XML layout files, class names, configuration structs, or any implementation detail to write steps. If it is not visible on screen, it does not belong in a step.
...
| `--product` | Demo app (Android) | Module path |
|---|---|---|
| `GiniBankSDKExample` | Gini Bank | `bank-sdk/example-app` |
| `GiniHealthSDKExample` | Gini Health | `health-sdk/example-app` |
