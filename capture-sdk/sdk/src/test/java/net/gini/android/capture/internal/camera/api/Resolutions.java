...
final class Resolutions {
    ...
    static List<Size> toSizesList(final int[][] resolutions) {
        final List<Size> sizes = new ArrayList<>(resolutions.length);
        for (final int[] resolution : resolutions) {
            sizes.add(toGiniSize(resolution));
        }
        return sizes;
    }

    static Size toGiniSize(final int[] resolution) {
