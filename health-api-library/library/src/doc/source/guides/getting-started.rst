...
If your app authenticates the Gini Health API requests itself (for example through your own backend), you don't
...
- ``SessionManager.getSession()`` is now called once per HTTP request instead of once per document manager
  operation. Operations which perform multiple HTTP requests (for example creating a partial document, or
  fetching the payment providers which downloads one icon per provider) call it multiple times, so a custom
  ``SessionManager`` should return a cached session while it is valid instead of authenticating on every
  call (the default anonymous session manager already does this).
