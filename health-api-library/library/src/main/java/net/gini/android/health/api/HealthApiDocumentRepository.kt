...
import net.gini.android.health.api.models.PaymentProvider.Companion.ANDROID_PLATFORM
...
class HealthApiDocumentRepository(
    ...
    suspend fun getPaymentProviders(): Resource<List<PaymentProvider>> {
        return withAccessToken { accessToken ->
            wrapInResource {
                documentRemoteSource.getPaymentProviders(accessToken).toMutableList()
                    .apply { add(0, PaymentProviderResponse(
                    ...
                    .filter { paymentProvider ->
                        paymentProvider.gpcSupportedPlatforms.map { it.lowercase(Locale.getDefault()) }.contains(ANDROID_PLATFORM)
                                || paymentProvider.openWithSupportedPlatforms.map { it.lowercase(Locale.getDefault()) }.contains(ANDROID_PLATFORM)
                    }
            }
        }
    }
